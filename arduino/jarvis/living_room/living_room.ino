...
#define HEATER_TEMP 4
#define ROOM_TEMP 2
#define ROOM_LIGHT 5
#define ROOM_TEMP_HUMIDITY_DHT11 3
#define BOARD_LED 13

// init libs
dht DHT;
OneWire heater_temp_sensor(HEATER_TEMP);
OneWire room_temp_sensor(ROOM_TEMP);
...
double frozen_actual_heater_temp = .0;
...
void loop() {
  ...
  if (millis() > timer) {
    ...
    actual_heater_temp += getTemp(heater_temp_sensor);
  
    // ***********************************************
    // Room temperature and humidity
    chk = DHT.read11(ROOM_TEMP_HUMIDITY_DHT11);
    ...
    if (ticks == tick_cnt) {
      ...
      frozen_actual_heater_temp = actual_heater_temp / tick_cnt;
      
      room_humidity = .0;
      actual_room_temp = .0;
      actual_heater_temp = .0;
  ...
  if (prepared) {
      ...
      if (send_ticks == 0) {
        ...
        switch (sent)
        {
          ...
          case 0:
            sendData("flat-room-humidity", String(frozen_room_humidity));
            //Serial.println("Room humidity: " + String(room_humidity / tick_cnt));
            break;
          case 1:
            sendData("flat-room-temp", String(frozen_actual_room_temp));
            //Serial.println("Room temperature " +  String(actual_room_temp / tick_cnt));
            break;
          case 2:
            sendData("flat-room-heat_temp", String(frozen_actual_heater_temp));
            //Serial.println("Room heater temperature " +  String(actual_heater_temp / tick_cnt));
            break;
          case 3:        
            sendData("flat-room-ref_temp", String(frozen_reference_room_temp));
            //Serial.println("Room reference temperature: " +  String(reference_room_temp / tick_cnt));            
            break;
          case 4:
            sendData("flat-room-light", String(frozen_room_light, 8));
...
//response = true;
  ...
  if (reply != 0 || (millis() > waited && prepared && !response)) {
      if (reply == 0) {
        Serial.println("due to wait");
      }
