...
#define BEDROOM_TEMP 3
#define BEDROOM_LIGHT 5
#define BEDROOM_TEMP_HUMIDITY_DHT11 2
#define BOARD_LED 13
...
// init libs
dht DHT;
OneWire room_temp_sensor(BEDROOM_TEMP);

// ethernet interface mac address, must be unique on the LAN
byte mymac[] = { 0x74, 0x69, 0x69, 0x2D, 0x30, 0x34 };
...
void loop() {
  ...
  if (millis() > timer) {
    ...
    chk = DHT.read11(BEDROOM_TEMP_HUMIDITY_DHT11);
    ...
    // ***********************************************
    // Light sensor
    room_light += analogRead(BEDROOM_LIGHT);
  ...
  if (prepared) {
    ...
    if (send_ticks == 0) {
      ...
      switch (sent)
      {
        ...
        case 0:
          sendData("flat-bedroom-humidity", String(frozen_room_humidity));
          //Serial.println("Room humidity: " + String(room_humidity / tick_cnt));
          break;
        case 1:
          sendData("flat-bedroom-temp", String(frozen_actual_room_temp));
          //Serial.println("Room temperature " +  String(actual_room_temp / tick_cnt));
          break;
        case 2:
          sendData("flat-bedroom-ref_temp", String(frozen_reference_room_temp));
          //Serial.println("Room reference temperature: " +  String(reference_room_temp / tick_cnt));
          break;
        case 3:
          sendData("flat-bedroom-light", String(frozen_room_light, 8));
